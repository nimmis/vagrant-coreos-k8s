...
Vagrant.configure("2") do |config|
  ...
  (1..$num_nodes+$num_masters).each do |i|
    if i <= $num_masters then
      $h_name = "%s-%02d" % [$master_name_prefix, i]

      $vm_memory = $vm_m_memory
      $vm_cpus = $vm_m_cpus
    else
      $h_name = "%s-%02d" % [$node_name_prefix, i-$num_masters]

      $vm_memory = $vm_n_memory
      $vm_cpus = $vm_n_cpus
    end

    config.vm.define vm_name = $h_name do |config|
